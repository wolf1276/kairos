...
import { listProtocolPositionsForAgent, listProtocolPositionsForOwner } from '../protocolPositionService.js';
...
positionsRouter.get('/', (req, res) => {
  res.json({
    success: true,
    positions: listPositionsForOwner(req.auth!.publicKey),
    protocolPositions: listProtocolPositionsForOwner(req.auth!.publicKey),
  });
});
...
agentPositionsRouter.get('/:id/positions', (req, res) => {
  ...
  res.json({
    success: true,
    positions: listPositionsForAgent(req.params.id),
    protocolPositions: listProtocolPositionsForAgent(req.params.id),
  });
});
