...
async function runOne(provider: 'openrouter' | 'nvidia', model: string, scenario: Scenario): Promise<RunResult> {
  ...
  const impl =
    provider === 'openrouter'
      ...
      : new NvidiaProvider({ provider: 'nvidia', model, apiKey: NVIDIA_KEY, temperature: 0, maxTokens: 1200, timeoutMs: 20000, maxRetries: 0, structuredOutput: true, baseUrl: NVIDIA_BASE_URL });
...
async function main() {
  ...
  const jobs: { provider: 'openrouter' | 'nvidia'; model: string }[] = [
    ...OPENROUTER_FREE_MODELS.map((model) => ({ provider: 'openrouter' as const, model })),
